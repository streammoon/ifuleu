...
public class UserServiceImpl implements UserService{
     ...
     */
    @HystrixCommand(fallbackMethod = "fallbackM",commandKey = "users",groupKey = "user-one",
          commandProperties = {
//            @HystrixProperty(name="execution.isolation.strategy",value="THREAD"), //线程池
            ...
            @HystrixProperty(name="execution.isolation.thread.timeoutMillisenconds",value="SEM")
          },threadPoolKey = "queryContentshystrixJackPool",
            threadPoolProperties ={
              @HystrixProperty(name="coreSize",value="100")//不配置的话默认信号量和线程池都是10
            }
    )
    @Override
    public Object users() {

        return new Tang("111","tang333","boy");
    }

    public Object  fallbackM(){
